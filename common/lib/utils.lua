...
local _last_error = ''
local _last_errno = 0
local _last_tc = -1 -- time consumed for the last procedure

function set_last(errno, err, tc)
    _last_errno = errno or 0
    _last_error = err or ''
    _last_tc = tc or -1
end

function get_last()
    local _eno = _last_errno
    local _err = _last_error
    local _tc = _last_tc
    _last_errno = 0
    _last_error = ''
    _last_tc = -1
    return _eno, _err, _tc
end
...
function sendToRedis(premature, logData)
...
function get_ip_info(ip)
    local _a, _b, _c, _d = ip:match('(%d+).(%d+).(%d+).(%d+)')
    if _a and _b and _c and _d then
        local ipint = _a * 16777216 + _b * 65536 + _c * 256 + _d
        local conf = require('config.common')
        local redis_t = require('sad.redis')
        local redis = redis_t.conn(conf.REDIS_HOST_LOCAL, conf.REDIS_PORT_LOCAL)
        local data, err = redis:zrangebyscore('i2', ipint, '+inf', 'withscores', 'limit', '0', '1')
        if not err and data then
            if data[1] then
                local _p, _ipint, _area, _city, _carrier = string.match(data[1], '([t|s]):(%d+):(%d+):(%d+):(.+)')
                if _p == 't' or (_p == 's' and _ipint == ipint) then
                    return _area, _city, _carrier
                end
            end
        else
            ngx.log(ngx.ERR, err)
...
function whiteList(uid)
    local list = require('config.whitelist')
    local uids = list.uids --uid 白名单
